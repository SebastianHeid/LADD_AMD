...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=("Path to load a previously trained checkpoint"),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help=("Save checkpoints at every X updates"),
    )
    ...
    parser.add_argument(
        "--recon_lambda", type=float, default=1.0, help=("loss weight for recon")
    )
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Number of time steps to sample from the original 1000 time steps for training G. We train one-step model by default"
        ),
    )
    ...
    parser.add_argument(
        "--ckpt_folder", type=str, default="ckpts", help="path to save ckpts"
    )
    ...
    parser.add_argument(
        ...
        help=("pkl file for loading data paths"),
    )
    ...
    parser.add_argument(
        "--exp_name", type=str, default="default_exp_name", help="identify exp name"
    )
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if config.generator.warm_up and config.generator.num_ts == 4:
                if step < config.generator.warm_up_steps:
                    ...
                    ts_indices = torch.multinomial(
                        torch.tensor(config.generator.noise_prob),
                        num_samples=bsz,
                        replacement=True,
                    )
            else:
            ...
            if (
                ...
                noisy_model_input = noise_scheduler.add_noise(
                    latents, noises, timesteps_for_init_noise
                )
            else:
                noisy_model_input = noise_scheduler.add_noise(
                    latents, noises, timesteps
                )
            ...
            if phase == "G":
                ...
                with accelerator.accumulate(disc):
                    ...
                    noised_latents = noise_scheduler.add_noise(
                    ...
                    if config.misaligned_pairs_D and bsz > 1:
                        ...
                        noised_shifted_latents = noise_scheduler.add_noise(
                            shifted_latents,
                            torch.randn_like(shifted_latents),
                            timesteps_D_shifted_pairs,
                        )

                        noised_predicted_x0 = torch.concat(
                            [noised_predicted_x0, noised_shifted_latents], dim=0
                        )
                        timesteps_D_fake = torch.concat(
                            [timesteps_D_fake, timesteps_D_shifted_pairs]
                        )
                ...
                logs = {
                    ...
                    "loss_real": loss_real.detach().item(),
                    "lr": lr_scheduler.get_last_lr()[0],
                }
                progress_bar.set_postfix(**logs)
                accelerator.log(logs, step=global_step)
                ...
                if (
                    ...
                    try:
                        accelerator.save_state(
                            output_dir=save_path, safe_serialization=False
                        )
