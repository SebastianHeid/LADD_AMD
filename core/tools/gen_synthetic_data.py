...
parser.add_argument(
    ...
    default=None,
    required=True,
    help=("a list containing prompts for generating images"),
)
...
parser.add_argument(
    ...
    help=("base model identifier for huggingface"),
)
...
parser.add_argument(
    ...
    help=("root folder for generated data"),
)
...
parser.add_argument(
    ...
    type=int,
    default=64,
    help=("spatial size for noise. width and height are same."),
)
...
parser.add_argument(
    ...
    type=str,
    default="fp16",
    help=("data saving precision. choose between [fp16, fp32]"),
    choices=["fp32", "fp16"],
)
parser.add_argument(
...
parser.add_argument(
    "--laion", type=bool, default=True, help=("if LAION dataset is generated")
)
...
parser.add_argument(
    ...
    help=("Path where LAION dataset is stored"),
)
...
parser.add_argument(
    ...
    type=str,
    default="latent",
    help=("how to save the generate image."),
    choices=["latent", "pil"],
)
...
if args.laion:
    data_list = sorted(
        [f.stem for f in Path(args.laion_path + "images").glob("*.webp")]
    )
    for ind, data_name in tqdm(enumerate(data_list)):
        with open(
            args.laion_path + "metadata/" + data_name + ".txt", "r", encoding="utf-8"
        ) as file:
            prompt = file.read()
...
if args.base_model == "stabilityai/stable-diffusion-2-1-base":
    ...
    def encoding_func(prompt):
        ...
        encoder_hidden_states = (
            pipe.text_encoder(inputs.cuda(), return_dict=False)[0]
            .squeeze()
            .to(weight_type)
            .cpu()
        )
    ...
    def encoding_func(prompt):
        ...
        for prompt, tokenizer, text_encoder in zip(prompts, tokenizers, text_encoders):
            ...
            prompt_embeds = text_encoder(
                text_input_ids.cuda(), output_hidden_states=True
            )
        ...
        prompt_embeds = (
            torch.concat(prompt_embeds_list, dim=-1).squeeze().to(weight_type).cpu()
        )
        added_cond_kwargs = {
            "text_embeds": pooled_prompt_embeds.squeeze().to(weight_type).cpu(),
