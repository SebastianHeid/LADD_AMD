...
parser.add_argument(
    "--purge_missing_data",
    action="store_true",
    help=("whether purge the missing pairs"),
)
...
for fname in tqdm(os.listdir(os.path.join(args.root_folder, latent_folder))):
    ...
    if (
        os.path.exists(os.path.join(args.root_folder, latent_rel_path))
        and os.path.exists(os.path.join(args.root_folder, txt_emb_rel_path))
        and os.path.exists(os.path.join(args.root_folder, noise_rel_path))
    ):
        res_list.append((latent_rel_path, noise_rel_path, txt_emb_rel_path))
    else:
